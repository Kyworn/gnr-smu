...
OFFSETS = {
    ...
    0x0F8: ("SoC Temperature", "°C"),
    0x100: ("Tctl / CPU Die Temp", "°C"),
...
temp_soc = load_cpu[0x0F8] - idle[0x0F8]
print(
    f"-> SoC Temp a pris {temp_soc:+.2f} °C (Va monter un peu car le CPU chauffe la puce entière)"
)
tjmax = load_cpu[0x100]
print(f"-> TjMax Limit = {tjmax:.1f} °C (Doit rester fixe)")
temp_gfx = load_cpu[0x2E8] - idle[0x2E8]
print(
    f"-> GFX Junction a pris {temp_gfx:+.2f} °C (Va monter un peu par inertie thermique)"
)

print("\n=== RÉSULTAT DU TEST ===")
if cpu_power_spike > 20 and abs(igpu_clock_spike) < 50 and abs(igpu_a_spike) < 3:
    print(
        "[SUCCESS] Isolations parfaites confirmées ! Les offsets GFX/iGPU n'ont pas réagi au test CPU massif."
    )
else:
    print(
        "[WARNING] Croisement détecté ! Les valeurs de l'iGPU ont explosé sans raison graphique, l'offset est peut-être mixte."
    )
