...
Note: CPU Vcore (SVI3 VDDCR_CPU) is not exposed through hwmon on Zen 5.

The conventional SVI3 SMN addresses (0xE0080, 0xE00A0) return 0xFFFFFFFF and are not mapped.
...
### Non-zero SMN registers at idle (range 0x3B10500-0x3B10600)
...
| 0x3B10530 | 0x01 | MSG = 1 (last command: TestMessage) |
| 0x3B1054C | 0x01 | other RSP? |
| 0x3B10564 | 0x01 | other RSP? |
| 0x3B10570 | 0x01 | status |
| 0x3B10574 | 0x01 | alternative mailbox RSP (rejects power commands) |
| 0x3B1057C | 0x01 | primary mailbox RSP ✓ |
...
The accepted responses above are a historical snapshot, not an allowlist.
Subsequent dedicated probing recorded MP1 `0x58`–`0x6F` freezing this firmware;
the runtime blocks that entire range. See [Granite Ridge safety findings](FINDINGS.md#4c-msg-ids-0x580x6f-exploration-result).

## Turbostat (system idle, Brave using ~73% CPU)

| Metric | Value |
|----------|--------|
| Bzy_MHz avg | 5220–5392 MHz |
| PkgWatt | 31–103 W (varies with Brave load) |
| LLC%hit | ~77–81% |
| IPC | ~0.84–0.94 |
| C3% (idle) | ~70–80% at rest |
...
These bits probably remain from the previous session's PPT=0 mW event (before
the reboot); they are not evidence of a current fault.

## Restore / Recovery

```bash
# Historical recovery command; dangerous writer, do not run casually
sudo python3 research/dangerous/smu_send.py reset
# = PPT 162W + TDC 120A + EDC 180A (stock; 160/220 are PBO figures, not a reset)

# If the CPU remains at 606 MHz after reset, reboot (all settings are volatile)
sudo reboot
```

## Default BIOS values

Corrected 2026-08-26: the table below was previously estimated and wrong on
three rows. All three limits and TjMax are directly readable from the PM table;
no estimate is needed.

| Limit | Value | Source |
|--------|--------|--------|
| PPT | 162 W | `d[2]`, = spec stock 9800X3D |
| TDC | 120 A | `d[8]` (previously recorded as 85 A) |
| EDC | 180 A | `d[63]` (previously recorded as 120 A) |
| TjMax | 88 °C | `d[10]` (previously recorded as 85 °C) |
...
3. **`turbostat Bzy_MHz` is real hardware telemetry** (APERF/MPERF).
