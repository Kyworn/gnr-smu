...
def avg_table(n=25, gap=0.12):
    ...
    for _ in range(n):
        k10.append(float(rd(K10_TCTL)) / 1000)
...
def main():
    global K10_TCTL, AMDGPU_VDDGFX, AMDGPU_VDDNB, AMDGPU_SCLK
    fails.clear()
    warns.clear()
    profile, why = get_hardware_profile()
    if profile != EXPECTED_PROFILE:
        raise SystemExit(f"refusing 9800X3D experiment: {why}")
    k10 = hwmon_inputs("k10temp", {"tctl": ("temp", "Tctl")})
    amdgpu = hwmon_inputs("amdgpu", {
        "vddgfx": ("in", "vddgfx"),
        "vddnb": ("in", "vddnb"),
        "sclk": ("freq", "sclk"),
    })
    K10_TCTL = k10["tctl"]
    AMDGPU_VDDGFX = amdgpu["vddgfx"]
    AMDGPU_VDDNB = amdgpu["vddnb"]
    AMDGPU_SCLK = amdgpu["sclk"]
    ...
    def k10_burst(n=15, gap=0.12):
        ...
        return statistics.median(
            float(rd(K10_TCTL)) / 1000 for _ in range(n) if not time.sleep(gap))
    ...
    idle_vddgfx = float(rd(AMDGPU_VDDGFX)) / 1000
    idle_vddnb = float(rd(AMDGPU_VDDNB)) / 1000
    idle_sclk = float(rd(AMDGPU_SCLK)) / 1e6
    ...
    for _ in range(40):
        ...
        sys_v.append(float(rd(AMDGPU_VDDGFX)) / 1000)
        time.sleep(0.15)
